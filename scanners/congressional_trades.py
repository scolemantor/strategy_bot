"""Congressional trades scanner via housestockwatcher.com / senatestockwatcher.com.
...
trading edge over the market. Subsequent research has tempered the magnitude
but the persistence of the signal in public-disclosure data is well-supported.

STOCK Act mechanics: Members of Congress and senior staff must disclose
covered transactions within 45 days of the transaction (or 30 days of
becoming aware). Public PTR (Periodic Transaction Report) PDFs are scraped
by community projects (housestockwatcher.com / senatestockwatcher.com) into
JSON feeds. Both feeds are hosted on S3.
...
Honest limits:
  ...
  - Members file late or amend after deadlines; community feeds reflect this
    imperfectly.
  - Spouse/family member trades are disclosed but harder to attribute and
    may carry less signal than a member's own trades.
  - Community feed PDF -> JSON parsing occasionally produces malformed
    entries (typos, dash variants in amount strings).
  - Field naming differs between the House feed ("representative", "type")
    and the Senate feed ("senator", "type"). Handled by separate parsers.
...
class CongressionalTradesScanner(Scanner):
    ...
    def run(self, run_date: date) -> ScanResult:
        ...
        try:
            house_raw = self._fetch_feed(HOUSE_API_URL, "house.json")
        except Exception as e:
            log.exception("Failed to fetch House feed")
            return empty_result(self.name, run_date, error=f"house feed: {e}")

        try:
            senate_raw = self._fetch_feed(SENATE_API_URL, "senate.json")
        except Exception as e:
            log.exception("Failed to fetch Senate feed")
        ...
        for r in house_raw:
            t = self._parse_house_record(r)
            if t is not None:
                trades.append(t)
        for r in senate_raw:
            t = self._parse_senate_record(r)
            if t is not None:
                trades.append(t)
    ...
    def _fetch_feed(self, url: str, cache_name: str) -> List[Dict]:
        ...
        log.info(f"Fetching {cache_name} from {url}")
        resp = requests.get(
            url,
    ...
    def _parse_house_record(self, raw: Dict) -> Optional[CongressionalTrade]:
        try:
            ...
            txn_date = self._parse_date(raw.get("transaction_date"))
            disc_date = self._parse_date(raw.get("disclosure_date"))
    ...
    def _parse_senate_record(self, raw: Dict) -> Optional[CongressionalTrade]:
        try:
            ticker = (raw.get("ticker") or "").strip().upper()
